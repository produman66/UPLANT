...
public class Cartochka5Adapter extends ListAdapter<MyPlantModel, Cartochka5ViewHolder> {

    public Cartochka5Adapter(@NonNull DiffUtil.ItemCallback<MyPlantModel> diffCallback) {
        super(diffCallback);
    }
    @NonNull
    @Override
    public Cartochka5ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return Cartochka5ViewHolder.create1(parent);
    }

    @Override
    public void onBindViewHolder(@NonNull Cartochka5ViewHolder holder, int position) {
        MyPlantModel current = getItem(position);
        holder.bind1(current.getPlantname(), current.getNameimage(), current.getRazmnozh());
    }
